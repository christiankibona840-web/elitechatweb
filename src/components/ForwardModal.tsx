    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-popover border border-border rounded-2xl w-[360px] max-w-[90vw] shadow-2xl max-h-[70vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-border">
          ...
          <button onClick={onClose} className="text-wa-icon hover:text-foreground"><X size={18} /></button>
        </div>
          ...
          {targets.map(t => {
            ...
            return (
              <button key={key} onClick={() => setSelected(key)}
                className={`flex items-center gap-3 px-4 py-2.5 w-full text-left transition-colors ${selected === key ? 'bg-primary/10' : 'hover:bg-wa-input-bg'}`}>
        ...
        <div className="p-4 border-t border-border flex justify-end">
          <button onClick={forward} disabled={!selected || sending}
            className="bg-primary text-primary-foreground rounded-lg px-4 py-2 text-sm font-semibold flex items-center gap-2 disabled:opacity-50 hover:bg-wa-green-dark transition-colors">
