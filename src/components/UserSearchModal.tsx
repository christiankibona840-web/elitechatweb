...
const UserSearchModal = ({ me, onClose, onStartChat }: UserSearchModalProps) => {
  ...
  const addAndChat = async (profile: Profile) => {
    ...
    toast.success(`✅ ${profile.display_name} ameongezwa`);
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-popover border border-border rounded-2xl w-[400px] max-w-[90vw] shadow-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-5 pb-3.5 border-b border-border">
          <h3 className="text-sm font-semibold text-foreground">🔍 Tafuta Watumiaji</h3>
        ...
        <div className="px-5 py-3">
          <div className="flex items-center gap-2 bg-wa-input-bg rounded-3xl px-3.5 py-2">
            ...
            <input
              ...
              placeholder="Tafuta kwa jina, username, au ID..."
              value={query}
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-3">
          {loading && <div className="text-center text-muted-foreground text-sm py-4">Inatafuta...</div>}
          {!loading && query.length >= 2 && results.length === 0 && (
            <div className="text-center text-muted-foreground text-sm py-6">Hakuna matokeo</div>
          )}
          {results.map(profile => (
            <button
              key={profile.id}
              onClick={() => addAndChat(profile)}
              className="flex items-center gap-3 px-3 py-2.5 w-full text-left hover:bg-wa-input-bg rounded-lg transition-colors"
            >
          ...
          {!loading && query.length < 2 && (
            <div className="text-center text-muted-foreground text-xs py-6">
              Andika herufi 2+ kutafuta watumiaji wote waliojisajili
            </div>
          )}
        </div>
      </div>
